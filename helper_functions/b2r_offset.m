function newmap = b2r_offset(cmin_input,cmax_input,apply_to_current_axis,reversed)
...
%
%   Additional edits by cjones: Can recover original by setting
%   apply_to_current_axis = true (default: false)
%   reverse (default = false): if true, reverse red to blue
%   
%   Additional edits by bsutherland: To allow for center not at 0
%   Examples:
%   ------------------------------
%   figure
%   peaks;
...
%% check the input
% assert(nargin==2,'b2r:nargin','two inputs required') % -cjones
assert(nargin>=2,'b2r:nargin','at least two inputs required') % +cjones
assert(cmin_input<cmax_input,'b2r:order','cmin must be less than cmax')

%% control the figure caxis 
lims = get(gca, 'CLim');   % get figure caxis formation

% cjones: added conditional to avoid messing with previous caxis
if exist('apply_to_current_axis','var') && apply_to_current_axis
...
if exist('reversed','var') && reversed
    color_input = [red_top; white_middle; blue_bottom];
end
