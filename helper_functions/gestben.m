...
for knf=1:length(nfft)
    ...
    [tmp(knf).P,tmp(knf).F]=tfestimate(x,y,win,[],nfft(knf),Fsamp);
    [tmp(knf).C,tmp(knf).F]=mscohere(x,y,win,[],nfft(knf),Fsamp);
    [tmp(knf).X,tmp(knf).F]=pwelch(x,win,[],nfft(knf),Fsamp);
    [tmp(knf).Y,tmp(knf).F]=pwelch(y,win,[],nfft(knf),Fsamp);
%     figure()
%     pwelch(y,win,[],nfft(knf),Fsamp);
