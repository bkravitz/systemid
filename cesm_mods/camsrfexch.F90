...
!
  ...
  public sysid_out_t ! added by Ben
...
! ADDED BY BEN
  type sysid_out_t    
     integer  :: lchnk                   ! chunk index
     integer  :: ncol                    ! number of active columns
     real(r8) :: tsys(pcols,1825)        ! mods for skin temperature
  end type sysid_out_t
! END BLOCK ADDED BY BEN
...
! !INTERFACE
!
!  subroutine hub2atm_alloc( cam_in ) ! modified by Ben
  subroutine hub2atm_alloc( cam_in, sysid_out )
...
!
   ...
   type(sysid_out_t), pointer ::  sysid_out(:)     ! Merged surface state
!
!!LOCAL VARIABLES:
!
...
! BLOCK ADDED BY BEN
    allocate (sysid_out(begchunk:endchunk), stat=ierror)
    if ( ierror /= 0 )then
      write(iulog,*) 'Allocation error: ', ierror
      call endrun('HUB2ATM_ALLOC error: allocation error for sysid')
    end if
! END BLOCK ADDED BY BEN
...
! BLOCK ADDED BY BEN
    do c = begchunk,endchunk
       sysid_out(c)%lchnk = c
       sysid_out(c)%ncol  = get_ncols_p(c)
       sysid_out(c)%tsys    (:,:) = 0._r8
    end do
! END BLOCK ADDED BY BEN
