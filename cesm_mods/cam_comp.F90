...
!-----------------------------------------------------------------------
   ...
   use spmd_utils,        only: masterproc, mpir8, mpicom
   use abortutils,        only: endrun
   use camsrfexch,        only: cam_out_t, cam_in_t, sysid_out_t ! last one added by Ben
...
subroutine cam_init( cam_out, cam_in, sysid_out, mpicom_atm, &
...
!   use shr_orb_mod,      only: shr_orb_params
   ...
   use units, only: getunit ! added by Ben
   use phys_grid, only: get_ncols_p, get_lat_all_p, get_lon_all_p ! added by Ben
   ...
   type(sysid_out_t), pointer :: sysid_out(:) ! added by Ben
   integer, dimension(144) :: lats ! added by Ben, number is hardcoded
   integer, dimension(144) :: lons ! added by Ben, number is hardcoded
   real(r8), dimension(13824,1830) :: sysid_readin ! added by Ben, numbers hardcoded
   real(r8), dimension(13824*1830) :: sysid_readin_temp ! added by Ben, numbers hardcoded
   real*4 :: data ! added by Ben
   character(len=10) :: blah
   integer :: i, j, k, benn, benm, c, unitm, ncol ! added by Ben
   !
   ! Local variables
   !
...
!      call hub2atm_alloc(cam_in)
      call hub2atm_alloc(cam_in,sysid_out)
...
!      call hub2atm_alloc( cam_in )
   ...
   ! code block added by Ben to read in system ID binary file
    if (masterproc) then
	write(iulog,*) 'Read in system ID binary file'
        sysid_readin(:,:)=0.0_r8
        sysid_readin_temp(:)=0.0_r8
        unitm = getunit()
	open(unitm,file='sysidout.bin',status='unknown',form='unformatted', &
	  access='direct',recl=4)
	do i=1,13824*1830 ! these numbers are hardcoded
	    read(unitm,rec=i) data
	    sysid_readin_temp(i)=dble(data)
	end do
	close(unitm)
	sysid_readin = reshape(sysid_readin_temp,(/13824,1830/))
        write(iulog,*) 'Populating system ID array'
    end if

    
! need barrier so chunks assigned to other threads don't populate with
! zeros before the file has been read in
    call mpibarrier(mpicom)
    call mpibcast(sysid_readin,13284*1830,mpir8,0,mpicom)
    call mpibarrier(mpicom)
	    ...
	    do i=1,ncol
	      j=(lons(i)-1)*96+lats(i)
	      do k=1,1825
		  sysid_out(c)%tsys(i,k)=sysid_readin(j,k+5)
	      end do
            end do
        end do

    if (masterproc) then
        write(iulog,*) 'Done with systems ID block in cam_init'
    end if
    ! end systems ID code block
...
subroutine cam_run2( cam_out, cam_in,sysid_out )
!-----------------------------------------------------------------------
!
...
!-----------------------------------------------------------------------
   ...
   use time_manager,     only: is_first_step, is_first_restart_step, &
                  get_nstep, get_step_size ! this line added by Ben
   use phys_grid, only: get_ncols_p ! added by Ben
   use ppgrid, only:  pver ! added by Ben
   ...
   type(sysid_out_t),  intent(in) :: sysid_out(begchunk:endchunk)

   integer :: c,ncol,dtime2,nstep2,i ! added by Ben
   ...
   if (offline_driver_dorun) then
	  ...
	  ! sysid block added by Ben
          dtime2 = get_step_size()          
          nstep2 = get_nstep()
!         if (masterproc) then
!          write(iulog,*) "Doing Systems ID in main loop, nstep=", nstep
!	 end if
	  do c=begchunk,endchunk
	    ncol=get_ncols_p(c)
	    do i=1,ncol
	      phys_state(c)%t(i,pver)=phys_state(c)%t(i,pver)+ &
	        sysid_out(c)%tsys(i,nstep2*dtime2/86400)
	      ! note integer division
	    end do
	  end do
	 ! end sysid block added by Ben
